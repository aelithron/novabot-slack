...
export default async function dailyRecap(app: App) {
	...
	const publicRecap = await app.client.chat.postMessage({ channel: config.channels.primary, text: `hi <@${config.owner.userID}>, daily recap time! how was your day? :3`, blocks: recapBlocks });
	if (!publicRecap.ok) {
...
export async function privateRecap(app: App, permaLink?: string) {
	const config = getConfig();
	if (!config.channels.secondary) return;
	let status = null;
	if (config.owner.email) {
		try {
			status = await getUniversalStatus(config.owner.email);
	...
	const privateRecap = await app.client.chat.postMessage({ channel: config.channels.secondary, text: `<@${config.owner.userID}>, private recap time! :3\n${permaLink ? `while you wait, you can look at <${permaLink}|the public recap>!` : ""}`, blocks: recapBlocks });
	if (!privateRecap.ok) {
