export async function getUniversalStatus(email: string): Promise<{ status: string, emoji: string, expiry: Date | null, setAt: Date } | null> {
  ...
  if (body.status === "No status set.") return null;
